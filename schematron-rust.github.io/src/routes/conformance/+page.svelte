<script lang="ts">
  ...
  const AREAS: Row[] = [
    ...
    { area: 'XPath 3.0 function items, =>, ||, !, let, Q{uri}local, the higher-order sequence functions', status: 'Subset', note: 'See spec/xpath3/.' },
...
<section class="section prose">
  ...
  <p>
    ...
    operator <code>!</code>, the <code>let</code> expression, EQNames
    (<code>Q&#123;uri&#125;local</code>, naming an element or attribute by
    namespace URI directly — node name tests only, not variable or function
    names), and the rest of the higher-order sequence function library
    (<code>filter</code>, <code>fold-left</code>, <code>fold-right</code>,
    <code>for-each-pair</code>, <code>function-lookup</code>,
    <code>function-arity</code>, <code>function-name</code>). As with
    XPath 2.0, everything outside the documented subset is a hard error
